...
interface BranchQuantums {
  branch: string;
  quantums: { [noteId: string]: any };
}

const FourthYearQuantums: React.FC = () => {
  const [branches, setBranches] = useState<BranchQuantums[]>([]);
  ...
  useEffect(() => {
    const quantumsRef = dbRef(db, 'version12/Materials/Quantum/Fourth_Year');
    const unsubscribe = onValue(quantumsRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        const branchList: BranchQuantums[] = Object.keys(data).map((branch) => ({
          branch,
          quantums: data[branch],
        }));
  ...
  const onFinishStep2 = async (values: any) => {
    ...
    // The path for the new subject will be: version12/Materials/Quantums/Fourth_Year/<newBranchName>
    const branchPath = `version12/Materials/Quantum/Fourth_Year/${newBranchName}`;
  ...
  const onFinishSubject = async (values: any) => {
    ...
    const encryptedSubName = encryptAES(sub_name);
    const encryptedBy = encryptAES(by);
    const branchPath = `version12/Materials/Quantum/Fourth_Year/${selectedBranch}`;
...
export default FourthYearQuantums;
