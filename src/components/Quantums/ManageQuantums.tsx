...
const ManageQuantum: React.FC = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      message.success('Quantum updated successfully');
      setEditModalVisible(false);
      handleApply();
    } catch (error) {
      message.error('Failed to update quantum');
    }
  };
  ...
  const handleDelete = async (noteId: string) => {
    ...
    try {
      ...
      message.success('Quantum deleted successfully');
      handleApply();
    } catch (error) {
      message.error('Failed to delete quantum');
    }
  };

  const filteredQuantum = quantumData.filter((note) => {
    ...
    { title: 'Dislikes', dataIndex: 'dislikes', key: 'dislikes' },
    {
      ...
      render: (_: any, record: any) => (
        <div className="flex items-center gap-2">
          ...
          <Popconfirm
            title="Are you sure you want to delete this quantum?"
  ...
  return (
    <div style={{ padding: '24px', background: '#fff', minHeight: '80vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
        ...
        <div style={{ fontWeight: 'bold' }}>Total Quantums: {totalQuantumCount}</div>
      </div>
