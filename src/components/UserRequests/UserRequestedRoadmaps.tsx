...
const UserRequestedRoadmaps: React.FC = () => {
  ...
  const [allData, setAllData] = useState<RoadmapRecord[]>([]);
  const [filteredData, setFilteredData] = useState<RoadmapRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedItem, setSelectedItem] = useState<RoadmapRecord | null>(null);
  ...
  // 2) Load & decrypt roadmap requests, then map in user info
  useEffect(() => {
    const roadmapRef = dbRef(db, 'version12/UserRequests/Roadmaps');
    const unsub = onValue(
      roadmapRef,
      (snap) => {
        const raw = snap.val() || {};
        const parsed: RoadmapRecord[] = Object.entries(raw).map(
          ([id, entry]: [string, any]) => {
      ...
      (err) => {
        ...
        message.error('Failed to load roadmap requests.');
        setLoading(false);
      }
    );
    return () => unsub();
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      await update(dbRef(db, `version12/UserRequests/Roadmaps/${id}`), {
...
export default UserRequestedRoadmaps;
