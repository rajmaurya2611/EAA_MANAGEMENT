...
const ManageBooks: React.FC = () => {
  ...
  const handleApply = () => {
    ...
    onValue(booksRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        const booksList = Object.values(data).map((note: any) => ({
          key: note.id,
          id: note.id,
        ...
        setTotalBooksCount(booksList.length);
      } else {
        setBooksData([]);
  ...
  const filteredBooks = booksData.filter((book) => {
    ...
    return (
      book.sub_name.toLowerCase().includes(searchLower) ||
      book.sub_code.toLowerCase().includes(searchLower) ||
      book.by.toLowerCase().includes(searchLower)
    );
  });
  ...
  return (
    <div style={{ padding: '24px', background: '#fff', minHeight: '80vh' }}>
      ...
      {/* Selected info and total books */}
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
        <div>
          <strong>Selected:</strong> {selectedYear.replace('_', ' ')} / {selectedBranch || 'None'}
        </div>
        <div style={{ fontWeight: 'bold' }}>Total Books: {totalBooksCount}</div>
      </div>

      <Spin spinning={loading}>
        <Table columns={columns} dataSource={filteredBooks} pagination={{ pageSize: 8 }} />
      </Spin>
