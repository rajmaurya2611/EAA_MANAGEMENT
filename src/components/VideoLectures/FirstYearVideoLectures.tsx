...
const FirstYearVideoLectures: React.FC = () => {
  ...
  // listen for all branches under Lectures → First_Year
  useEffect(() => {
    const lecturesRef = dbRef(db, 'version12/Materials/Lectures/First_Year');
  ...
  const onFinishStep2 = async (vals: any) => {
    ...
    const path = `version12/Materials/Lectures/First_Year/${newBranchName}`;
  ...
  const onFinishLecture = async (vals: any) => {
    ...
    const path = `version12/Materials/Lectures/First_Year/${selectedBranch}`;
    const newRef = push(dbRef(db, path));
    encrypted.id = newRef.key!;
...
export default FirstYearVideoLectures;
