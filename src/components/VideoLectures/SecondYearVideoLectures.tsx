...
const SecondYearVideoLectures: React.FC = () => {
  ...
  // listen for all branches under Lectures → Second_Year
  useEffect(() => {
    const lecturesRef = dbRef(db, 'version12/Materials/Lectures/Second_Year');
  ...
  const onFinishStep2 = async (vals: any) => {
    ...
    const path = `version12/Materials/Lectures/Second_Year/${newBranchName}`;
  ...
  const onFinishLecture = async (vals: any) => {
    ...
    const path = `version12/Materials/Lectures/Second_Year/${selectedBranch}`;
    const newRef = push(dbRef(db, path));
    encrypted.id = newRef.key!;
...
export default SecondYearVideoLectures;
