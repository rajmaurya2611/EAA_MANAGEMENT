...
const FourthYearPYQ: React.FC = () => {
  ...
  // ----------------------- Data Listener -----------------------
  useEffect(() => {
    const PyqsRef = dbRef(db, 'version12/Materials/Pyq/Fourth_Year');
    const unsubscribe = onValue(PyqsRef, (snapshot) => {
      ...
      if (data) {
        ...
        // Default to Fourth branch and its Fourth year if nothing is selected.
  ...
  const onFinishBranchStep3 = async (values: any) => {
    ...
    const branchPath = `version12/Materials/Pyq/Fourth_Year/${newBranchName}/${newYearForBranch}`;
  ...
  const onFinishSubject = async (values: any) => {
    ...
    const encryptedSubName = encryptAES(sub_name);
    const encryptedBy = encryptAES(by);
    const branchPath = `version12/Materials/Pyq/Fourth_Year/${selectedBranch}/${selectedYear}`;
  ...
  const onFinishAddYearStep2 = async (values: any) => {
    ...
    const encryptedSubName = encryptAES(sub_name);
    const encryptedBy = encryptAES(by);
    const branchPath = `version12/Materials/Pyq/Fourth_Year/${selectedBranch}/${newYear}`;
    const newNoteRef = push(dbRef(db, branchPath));
    const newNoteId = newNoteRef.key;
    const noteData = {
      by: encryptedBy,
