...
const ManageSyllabus: React.FC = () => {
  ...
  const [totalSyllabusCount, setTotalSyllabusCount] = useState<number>(0);

  const years = ['First_Year', 'Second_Year', 'Third_Year', 'Fourth_Year'];

  useEffect(() => {
    if (selectedYear) {
      const yearRef = dbRef(db, `version12/Materials/Syllabus/${selectedYear}`);
  ...
  const handleApply = () => {
    ...
    setLoading(true);
    const syllabusRef = dbRef(db, `version12/Materials/Syllabus/${selectedYear}/${selectedBranch}`);
    onValue(syllabusRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        const syllabusList = Object.values(data).map((note: any) => ({
          key: note.id,
          id: note.id,
        ...
        setTotalSyllabusCount(syllabusList.length);
      } else {
        setSyllabusData([]);
        setTotalSyllabusCount(0);
      }
      setLoading(false);
    });
  };
  ...
  const handleUpdate = async () => {
    try {
      ...
      const path = `version12/Materials/Syllabus/${selectedYear}/${selectedBranch}/${editingNote.id}`;
      const updatedData = {
        id: editingNote.id,
  ...
  const handleDelete = async (noteId: string) => {
    const path = `version12/Materials/Syllabus/${selectedYear}/${selectedBranch}/${noteId}`;
    try {
  ...
  const filteredSyllabus = syllabusData.filter((note) => {
    const searchLower = searchText.toLowerCase();
    return (
      note.sub_name.toLowerCase().includes(searchLower) ||
      note.sub_code.toLowerCase().includes(searchLower) ||
      note.by.toLowerCase().includes(searchLower)
    );
  });
  ...
  return (
    <div style={{ padding: '24px', background: '#fff', minHeight: '80vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
        ...
        <div style={{ fontWeight: 'bold' }}>Total Syllabus: {totalSyllabusCount}</div>
      </div>

      <Spin spinning={loading}>
        <Table columns={columns} dataSource={filteredSyllabus} pagination={{ pageSize: 8 }} />
      </Spin>
