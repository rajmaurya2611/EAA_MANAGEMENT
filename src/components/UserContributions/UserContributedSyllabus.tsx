...
const ManageUserContributedSyllabus: React.FC = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, 'version12/Contribution/Syllabus');
  ...
  useEffect(() => {
    ...
    const path = `version12/Contribution/Syllabus/${selectedYear}/${selectedBranch}`;
  ...
  const handleToggle = (rec: any, field: 'isVisible' | 'isVerified' | 'isDeleted') => {
    const path = `version12/Contribution/Syllabus/${rec.year}/${rec.branch}/${rec.id}`;
  ...
  const handleModalSave = async () => {
    ...
    const path = `version12/Contribution/Syllabus/${selectedItem.year}/${selectedItem.branch}/${selectedItem.id}`;
  ...
  return (
    <div style={{ padding: 24 }}>
      <h2 className="text-xl font-semibold mb-4">📘 Manage Syllabus Contributions</h2>
...
export default ManageUserContributedSyllabus;
