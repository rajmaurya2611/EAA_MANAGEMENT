...
const ManageUserContributedQuantums: React.FC = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, 'version12/Contribution/Quantum');
  ...
  useEffect(() => {
    ...
    const path = `version12/Contribution/Quantum/${selectedYear}/${selectedBranch}`;
  ...
  const handleToggle = (rec: any, field: 'isVisible' | 'isVerified' | 'isDeleted') => {
    const path = `version12/Contribution/Quantum/${rec.year}/${rec.branch}/${rec.id}`;
  ...
  const handleModalSave = async () => {
    ...
    const path = `version12/Contribution/Quantum/${selectedItem.year}/${selectedItem.branch}/${selectedItem.id}`;
  ...
  return (
    <div style={{ padding: 24 }}>
      <h2 className="text-xl font-semibold mb-4">📘 Manage Quantums Contributions</h2>
...
export default ManageUserContributedQuantums;
