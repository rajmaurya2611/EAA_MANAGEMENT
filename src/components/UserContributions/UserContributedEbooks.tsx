...
const ManageUserContributedEbooks: React.FC = () => {
  ...
  useEffect(() => {
    const ref = dbRef(db, 'version12/Contribution/E-Book');
  ...
  useEffect(() => {
    ...
    const path = `version12/Contribution/E-Book/${selectedYear}/${selectedBranch}`;
  ...
  const handleToggle = (rec: any, field: 'isVisible' | 'isVerified' | 'isDeleted') => {
    const path = `version12/Contribution/E-Book/${rec.year}/${rec.branch}/${rec.id}`;
  ...
  const handleModalSave = async () => {
    ...
    const path = `version12/Contribution/E-Book/${selectedItem.year}/${selectedItem.branch}/${selectedItem.id}`;
  ...
  return (
    <div style={{ padding: 24 }}>
      <h2 className="text-xl font-semibold mb-4">📘 Manage E-Books Contributions</h2>
...
export default ManageUserContributedEbooks;
