...
const FourthYearNotes: React.FC = () => {
  ...
  useEffect(() => {
    const notesRef = dbRef(db, 'version12/Materials/Notes/Fourth_Year');
  ...
  const onFinishStep2 = async (values: any) => {
    ...
    // The path for the new subject will be: version12/Materials/Notes/Fourth_Year/<newBranchName>
    const branchPath = `version12/Materials/Notes/Fourth_Year/${newBranchName}`;
  ...
  const onFinishSubject = async (values: any) => {
    ...
    // Encrypt the PDF link before saving
    const encryptedPdf = encryptAES(pdf);
    const branchPath = `version12/Materials/Notes/Fourth_Year/${selectedBranch}`;
      ...
      <Layout style={{ padding: '24px' }}>
        ...
        <div className="text-left mb-7">
              ...
              <p className="text-sm text-gray-500">
                  Note: The Content will be added in 4th Year (<span className="font-medium text-gray-500">{selectedBranch}</span>)
              </p>
        </div>
...
export default FourthYearNotes;
