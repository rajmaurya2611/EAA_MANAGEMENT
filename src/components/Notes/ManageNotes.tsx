...
const ManageNotes: React.FC = () => {
  ...
  const handleApply = () => {
    ...
    onValue(notesRef, (snapshot) => {
      ...
      if (data) {
        ...
        setTotalNotesCount(notesList.length);
      } else {
        setNotesData([]);
  ...
  return (
    <div style={{ padding: '24px', background: '#fff', minHeight: '80vh' }}>
      <h2 className="text-xl font-bold mb-4">Manage Notes</h2>
      ...
      {/* Selected info and total notes */}
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
        <div>
          <strong>Selected:</strong> {selectedYear.replace('_', ' ')} / {selectedBranch || 'None'}
        </div>
        <div style={{ fontWeight: 'bold' }}>Total Notes: {totalNotesCount}</div>
      </div>
