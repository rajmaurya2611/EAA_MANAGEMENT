...
const FirstYearNotes: React.FC = () => {
  ...
  useEffect(() => {
    const notesRef = dbRef(db, 'version12/Materials/Notes/First_Year');
  ...
  const onFinishStep2 = async (values: any) => {
    ...
    // The path for the new subject will be: version12/Materials/Notes/First_Year/<newBranchName>
    const branchPath = `version12/Materials/Notes/First_Year/${newBranchName}`;
  ...
  const onFinishSubject = async (values: any) => {
    ...
    // Encrypt the PDF link before saving
    const encryptedPdf = encryptAES(pdf);
    const branchPath = `version12/Materials/Notes/First_Year/${selectedBranch}`;
      ...
      <Layout style={{ padding: '24px' }}>
        ...
        <div className="text-left mb-7">
              ...
              <p className="text-sm text-gray-500">
                  Note: The Content will be added in 1st Year (<span className="font-medium text-gray-500">{selectedBranch}</span>)
              </p>
        </div>
