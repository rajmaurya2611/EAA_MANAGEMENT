...
interface Placement_notesCategory {
  category: string;
  placement_notes: { [id: string]: any };
}

const NewPlacementNotes: React.FC = () => {
  const [categories, setCategories] = useState<Placement_notesCategory[]>([]);
  ...
  const [placement_notesModalForm] = Form.useForm();
  const [placement_notesForm] = Form.useForm();
  const [showSuccessModal, setShowSuccessModal] = useState<boolean>(false); // ✅ Success modal

  useEffect(() => {
    const placement_notesRef = dbRef(db, 'version12/Placement/Placement_Notes');
    const unsubscribe = onValue(placement_notesRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        const catList: Placement_notesCategory[] = Object.keys(data).map((cat) => ({
          category: cat,
          placement_notes: data[cat],
        }));
  ...
  const addplacement_notes = async (values: any, targetCategory: string) => {
    ...
    const placement_notesRef = push(dbRef(db, `version12/Placement/Placement_Notes/${targetCategory}`));
    const placement_notesId = placement_notesRef.key;

    const placement_notesData = {
      id: placement_notesId,
      by: encryptAES(by),
    ...
    try {
      await set(placement_notesRef, placement_notesData);
      message.success('Placement Notes added successfully!');
      placement_notesForm.resetFields();
      placement_notesModalForm.resetFields();
      setModalStep(1);
      ...
      console.error(err);
      message.error('Failed to add Placement Notes.');
    }
  };

  const onFinishStep2 = (values: any) => addplacement_notes(values, newCategoryName);
  const onFinishplacement_notes = (values: any) => addplacement_notes(values, selectedCategory);
      ...
      <Layout style={{ minHeight: '70vh' }}>
        ...
        <Layout style={{ padding: '24px' }}>
          <Content style={{ background: '#fff', padding: 24 }}>
            <h2>Add Placement Notes to {selectedCategory}</h2>
            <Form
              form={placement_notesForm}
              layout="vertical"
              onFinish={onFinishplacement_notes}
            ...
            >
              ...
              <Form.Item label="Subject Name" name="sub_name" rules={[{ required: true }]}>
                <Input placeholder="Placement Notes Title" />
              </Form.Item>
              <Form.Item>
                <Button type="primary" htmlType="submit">Add Placement Notes</Button>
              </Form.Item>
            </Form>
          </Content>
        </Layout>

        {/* Modal for adding a new category and placement_notes */}
        <Modal
          open={showAddModal}
          title="Add New Placement Notes"
        ...
        >
          <p>Your Placement Notes has been added successfully!</p>
        </Modal>
      </Layout>
    </>
  );
...
export default NewPlacementNotes;
